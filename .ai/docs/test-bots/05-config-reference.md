...
| `pair_policy` | str | как стратегия набирает пары: `manual` — явным списком, `volatility_jumps` — отбором по скачкам. Отбор принадлежит `legacy`, а не системе вообще |
...
## `StrategyPair` — `general/app/db/models.py`, `class StrategyPair`, таблица `strategy_pairs`

Набор пар одной стратегии. `watched_pair` остаётся общим техническим
списком питателя и собирается из этих наборов — `sync_watched_pairs`:

```
watched_pair = объединение strategy_pairs + пары активных ботов
```

Пара, нужная двум стратегиям, лежит двумя строками (уникальность по
`uq_strategy_pairs_key`): иначе пересборка набора одной унесла бы
котировки у другой. Пары активных ботов с закреплённым символом
досыпаются в общий список помимо наборов — пропадёт пара, и бот просто
встанет.
