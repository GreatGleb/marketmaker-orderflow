...
Дальше по этим сделкам считается прибыль каждой конфигурации
(`top_bots_report.py` — за сутки, неделю, две недели), а «копиботы» умеют
динамически подхватывать конфиг того бота, который был прибылен за последние
N минут. Сами сделки живут 72 часа, статистика по ним переживает чистку в
свёртках (`test_order_rollups`), см. [10-retention.md](10-retention.md).
...
| `general/app/dependencies.py` | `get_session`, `get_redis` (адрес Redis — `settings.REDIS_URL`, умолчание `redis://redis:6379/0`) |
