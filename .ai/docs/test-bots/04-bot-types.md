...
`new_bots.py` процентные боты не создаёт (код закомментирован, `new_bots.py:335-349`).
...
в `price_provider.py:42-47` и `exit_strategy.py:112-117`).

* **Вход** (`price_provider.py:41-102`): пересечение за последние 2 минуты.
...
`just_copy_bots=True`). Затем, поскольку у донора заполнено
`copy_bot_min_time_profitability_min`, проходит и ветку v1 — но уже с
пересчётом лидеров **на лету** (`demo_test_bot.py:142-154`), без Redis.

Итог — цепочка `бот → копибот v1 → копибот v2`. `new_bots.py` создаёт
20 копиботов v2 (по одному на окно), MA-боты — закомментированы (`new_bots.py:424-459`).

В `toDo.yml` висит открытый вопрос: «понять нужно ли в demo_test_bot
`update_config_from_referral_bot` — if `original_bot_config.copybot_v2_time_in_minutes`
is not None» — то есть двойной проход v2 через ветку v1 сам автор считает
спорным.
...
* **Выбор пары по волатильности.** `min_timeframe_asset_volatility` и Redis
  `most_volatile_symbol_{tf}`: код в `demo_test_bot.py:281-286` закомментирован,
