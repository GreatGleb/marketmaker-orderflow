...
Дополнительно все пути заполнения и пересборки проверяют возможность
MARKET-покупки с номиналом не выше `WATCHED_PAIR_MAX_BUY_NOTIONAL_USDT`
(по умолчанию 9 USDT). Недоступные старые и закреплённые пары исключаются.
Расчёт, свежесть данных и применение к текущему списку —
[отбор по бюджету](11-watched-affordability.md).
...
На всё время работы процесс держит в Redis флаг `test_bots_running:*`. Пока
он есть, `new_bots`, `seed_watched_pairs` и `init_setup` работать откажутся:
supervisorctl ручной запуск не остановит, а менять под ним парк ботов или
`watched_pair` — тихая порча данных. Кто держит флаг:

```bash
docker exec -it orderflow_general python -m app.scripts.simulator_flag
```

Флаг снимается на выходе, но `SIGTERM` и `SIGKILL` до этого не доходят: после
`kill` он висит ещё до минуты (TTL). Подробности — пункт 21
...
Ключ `copy_bot_*` публикуется раз в 30 секунд, а рейтинги кэшируются на
`max(30 с, 5% окна)`. Поэтому расхождение с точным окном не всегда ошибка.
Начиная с 2026-09-18 отсутствие кандидата отзывает ключ, TTL публикации —
90 секунд; ожидающий бот перепроверяет выбор до входа. Старые бессрочные
ключи без `published_at` не принимаются. После обновления сначала должен
отработать `set_profitable_bot`; миграция ключей вручную не нужна.

Места читаются так: 1 — точное совпадение, 2–3 — другой кандидат из отбора,
«не в отборе» / «донора не было» — расхождение с реконструкцией на момент
входа. Проверяйте штатный лаг рейтинга и дату версии: до исправления
воркер действительно оставлял прежний ключ, а бот не перечитывал его.
Пример подтверждённого многоминутного отставания — финальный аудит
[2026-09-18](../../audits/2026-09-18-final/report.md).
Отчёт считается по свёрткам; глубже рождения нынешнего парка он не идёт,
поскольку `new_bots.py` выдаёт номера ботов заново.
...
| `order_queue` = 0 и `test_orders` не растёт | симулятор висит: нет свежего `price_snapshot:{SYMBOL}` или пара не попала в `shared_data` |
...
| `❌ Не удалось найти реферального бота` | нет свежего выбора в `copy_bot_{id}`: отсутствуют кандидаты либо не работает `set_profitable_bot` |
